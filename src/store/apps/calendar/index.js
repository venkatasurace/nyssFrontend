...
export const addEvent = createAsyncThunk('appCalendar/addEvent', async (event, { dispatch }) => {
  ...
  await dispatch(fetchEvents(['Personal', 'Ganesh', 'Durga', 'Events', 'Amount']))
...
export const appCalendarSlice = createSlice({
  ...
  initialState: {
    events: [],
    selectedEvent: null,
    selectedCalendars: ['Personal', 'Ganesh', 'Durga', 'Events', 'Amount']
  },
  reducers: {
    ...
    handleAllCalendars: (state, action) => {
      ...
      if (value === true) {
        state.selectedCalendars = ['Personal', 'Ganesh', 'Durga', 'Events', 'Amount']
      } else {
